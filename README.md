...
## Windows
Перейди в `MSHP-IDE-Windows/` и запусти `МШПайтон.Оффлайн.bat`.

## macOS / Linux
Перейди в `MSHP-IDE-macOS/` или `MSHP-IDE-Linux/` и запусти `./МШПайтон.Оффлайн.sh`
(понадобится `chmod +x МШПайтон.Оффлайн.sh`).
